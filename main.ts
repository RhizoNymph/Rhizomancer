import { App, Setting, Notice, PluginSettingTab, Plugin, Modal, ItemView, TFile, TFolder, TAbstractFile, MarkdownView, WorkspaceLeaf } from 'obsidian';
...
export default class Rhizomancer extends Plugin {
    ...
    async promptForIndexAndExecute(action: (indexName: string) => Promise<void>) {
    ...
    async fetchIndexedHashes(): Promise<Set<string>> {
        try {
            const response = await fetch(`${this.settings.serverAddress}/get_indexed_hashes`);
    ...
    async indexPDF(file: TFile, indexName: string = 'universal') {
        ...
        if (indexedHashes.has(fileHash)) {
            console.log(`File ${file.name} found in indexed hashes. Not indexing.`);
            return;
        }

        console.log(`File ${file.name} not found in indexed hashes. Proceeding with indexing.`);

        try {
            const response = await fetch(`${this.settings.serverAddress}/indexPDF`, {
                method: 'POST',
                headers: {
            ...
            if (response.ok) {
                ...
                console.error(`Failed to index ${file.name}. Server responded with status ${response.status}`);
            }
        } catch (error) {
    ...
    async indexMarkdown(file: TFile, indexName: string = 'universal') {
        ...
        try {
            const response = await fetch(`${this.settings.serverAddress}/indexText`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
            ...
            console.error(`Failed to index markdown ${file.name} to ${indexName}:`, error);
...
class RhizomancerSettingTab extends PluginSettingTab {
    ...
    display(): void {
        ...
        new Setting(containerEl)
            ...
            .setDesc('The address of your Flask server (including protocol and port)')
            .addText(text => text
